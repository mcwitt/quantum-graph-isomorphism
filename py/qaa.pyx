...
def hamiltonian(
    ...
    cdef np.ndarray[np.double_t] vals = np.zeros(nnz, dtype=np.double)
    cdef double c
    cdef int j, k
    cdef UINT i, m, inz = 0
...
def mag_z(np.ndarray[np.double_t] psi):
    result = 0.
    for j in range(N): result += sigma_z(psi, j)
    return result / N

def mag_x(np.ndarray[np.double_t] psi):
    result = 0.
    for j in range(N): result += sigma_x(psi, j)
    return result / N

def overlap(np.ndarray[np.double_t] psi):
    result = 0.
    for j in range(1, N):
        for k in range(j):
...
cdef double sigma_z(np.ndarray[np.double_t] psi, int j):
    cdef double result = 0.
    cdef UINT i
    for i in range(len(psi)): result += psi[i]**2 * spin(i, j)
    return result

cdef double sigma2_z(np.ndarray[np.double_t] psi, int j, int k):
    cdef double result = 0.
    cdef UINT i
    for i in range(len(psi)): result += psi[i]**2 * spin(i, j) * spin(i, k)
    return result

cdef double sigma_x(np.ndarray[np.double_t] psi, int j):
    cdef double result = 0.
    cdef UINT i, m = 1UL << j
